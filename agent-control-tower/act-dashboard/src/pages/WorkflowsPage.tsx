...
export function WorkflowsPage() {
  ...
  return (
    <div style={{ padding: '24px 28px', maxWidth: 960, margin: '0 auto' }}>
      ...
      {/* M2: Merge workflow modal (replaces window.prompt) */}
      <div
        className={`modal-scrim${mergeModalOpen ? ' open' : ''}`}
        onClick={() => setMergeModalOpen(false)}
        aria-hidden={!mergeModalOpen}
      />
      <div
        className={`modal${mergeModalOpen ? ' open' : ''}`}
        role="dialog"
        aria-modal="true"
      ...
      {/* M2: Execute YAML modal (replaces window.prompt) */}
      <div
        className={`modal-scrim${yamlModalOpen ? ' open' : ''}`}
        onClick={() => setYamlModalOpen(false)}
        aria-hidden={!yamlModalOpen}
      />
      <div
        className={`modal${yamlModalOpen ? ' open' : ''}`}
        role="dialog"
        aria-modal="true"
