...
- **Authority for the verdicts in this file:** the fix-wave tree, `06caaf0` plus the seven review-driven
  fix commits `39583b1`, `de9e41f`, `da99c9f`, `b782c20`, `f03211c`, `33e1a25`, `6b48899`. The
  authoritative executions are **E11** (full matrix, `harness-console11.log`, `SUMMARY.run10.md`),
  **E12** (`harness-console12.log`, `SUMMARY.run11.md`, the environment-blocked steps re-run) and the
  E11 regression lane re-run (`harness-console13.log`) — all on that tree. Earlier executions are
  disclosure only.
...
| E13 | `harness-console13.log` | see the regression table | `6b48899` | playwright lane re-run | Tests whether the in-lane interference of two workflow specs is stable. |
...
|---|----------|---------|----------|
| — | preflight: health + credential + zero-credit pin | PASS | `preflight.run13.log` (E12); E11's `preflight.run12.log` also PASS |
| — | rebuild-image: pinned qoder sandbox image | PASS | `rebuild-image.run6.log` (E11) |
| S1 | ordinary run on the pinned zero-credit model | PASS | `S1.run6.log` (E11) |
| S2 | write → ask → approve once → executes; second write asks again | PASS | `S2.run6.log` (E11) |
| S3 | deny → no side effect | PASS | `S3.run6.log` (E11) |
| S4 | expiry → reject delivered, typed 409 on a late decide, no stale allow | PASS | `S4.run8.log` (E11) |
| S5 | cancel during a pending permission (sandbox survival) | PASS | `S5.run9.log` (E12; E11's attempt died in the sandbox-creation outage) |
| S6 | credential set/mask/remove/test | PASS | `S6.run6.log` (E11) |
| S7 | worker self-approval denied | PASS | `S7.run6.log` (E11) — the live-worker-token half remains NOT VERIFIED, see deltas |
| S8 | one-use write grant | PASS | `S8.run6.log` (E11) — the worker one-use replay half remains NOT VERIFIED, see deltas |
| S9 | Kanban-dispatched run; card moves; decision does not mark DONE | PASS | `S9.run9.log` (E12; E11's attempt died in the outage). Also the live exercise of the request-changes stop and the agent-slot hand-over |
| S10 | backend restart interrupts a pending ask | PASS | `S10.run12.log` (E12; carries `PACK_CREDENTIAL_KEY` across the in-harness restart) |
| S11 | platform-MCP read auto-allowed; write gated; executions run-correlated | PASS | `S11.run12.log` (E11) — with R81 and the whole approval fix wave in place |
| S12 | governance regression UI smoke | PASS | `S12.run11.log` (E11) |

## Regression lanes

| Lane | Command | Log | Verdict |
|------|---------|-----|---------|
| mvn-test | `mvn clean test -Dspring.profiles.active=h2` | `regression-mvn-test.run7.log` | **PASS** (E11) |
| mvn-verify | `mvn verify` | `regression-mvn-verify.run7.log` | **PASS** (E12). E11's occurrence failed in `act-mcp` Failsafe on `McpNoneModeIdentityIntegrationTest` with a Mockito `WrongTypeOfReturnValue: Boolean cannot be returned by getToken()` at `:88` — refuted as a regression: the wave changed exactly one file under `act-mcp/` (a string), `McpProperties` has no `getToken()`, and `mvn verify -pl act-mcp` alone is BUILD SUCCESS with that class 4/0 (`e11-postverify-act-mcp.run1.log`); E12's lane run then passed. |
| vitest | `pnpm test` | `regression-vitest.run5.log` | **PASS** (E11) |
| build | `pnpm build` | `regression-build.run5.log` | **PASS** (E11) |
| playwright | `npx playwright test` (QODER_E2E* stripped) | `regression-playwright.run7.log` (E13) | **PASS** — `0 failed / 2 flaky / 35 skipped / 221 passed (14.2 m)`, `EXIT=0` (the two flakes were retry-absorbed). The two earlier occurrences were both classified away from the code: E11's hard failure `overview-dashboard.spec.ts:28:3` was dirty-database slowness (the same spec passes in 2.0 s on a fresh database, `pw-triage-e11.run3.log`), and E12's `workflow-lifecycle-e2e.spec.ts:269` + `workflow-state-machine-e2e.spec.ts:159` pass standalone 27/27 in 31.9 s (`pw-triage-e12.run1.log`) — in-lane state interference between workflow specs, unrelated to the qoder surface, which did not recur in E13. |
| container-runtime | `pwsh … container-runtime-e2e.ps1 && bash e2e/container-runtime-e2e.sh` | `regression-container-runtime.run5.log` | **PASS** (E11) |
| leak-scan | count-only PAT grep over every evidence file | `leak-scan.run11.log` | **PASS** — 0 hits / 267 files (E12) |
