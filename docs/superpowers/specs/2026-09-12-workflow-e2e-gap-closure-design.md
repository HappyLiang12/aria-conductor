...
| 7 | `e2e/journey-crew-run-approve.spec.ts` (new) | A single UI-only journey: deploy an agent from the Crew catalog, start a run from it, and approve the resulting ask in the kanban Review column. |
